...
use serde_json::json;
use tables::common::{
    arity_distribution, collect_bits, read_tables, total_rows, write_json, Table,
};
use tables::rank_stats::{summarize_table_ranks, RankSummary};
use tables::triangle_consistency_filter::{
    filter_tables_by_triangle_consistency, TriangleConsistencyInfo, TriangleConsistencySettings,
};
...
struct Args {
    ...
    max_triangle_pairs_per_anchor: usize,
    min_shared_bits_between_outer_tables: usize,
    max_outer_pair_row_product: usize,
}

impl Default for Args {
    fn default() -> Self {
        let settings = TriangleConsistencySettings::default();
        Self {
            input: PathBuf::from(
                "runs/2026-04-05-main-chain-neighbor-consistency-fixed-point/tables.fixed_point.json",
            ),
            output: PathBuf::from(
                "runs/2026-04-05-triangle-consistency-filter/tables.fixed_point.json",
            ),
            report: PathBuf::from(
                "runs/2026-04-05-triangle-consistency-filter/report.json",
            ),
            max_rounds: None,
...
impl Args {
    fn parse() -> Result<Self> {
        ...
        while let Some(flag) = iter.next() {
            match flag.as_str() {
                ...
                "--max-neighbors-considered" => {
                    args.max_neighbors_considered =
                        expect_value(&mut iter, "--max-neighbors-considered")?
                            .parse()
                            .with_context(|| "invalid value for --max-neighbors-considered")?;
                }
                "--max-triangle-pairs-per-anchor" => {
                    args.max_triangle_pairs_per_anchor =
                        expect_value(&mut iter, "--max-triangle-pairs-per-anchor")?
                            .parse()
                            .with_context(|| "invalid value for --max-triangle-pairs-per-anchor")?;
                }
                "--min-shared-bits-between-outer-tables" => {
                    args.min_shared_bits_between_outer_tables =
                        expect_value(&mut iter, "--min-shared-bits-between-outer-tables")?
                            .parse()
                            .with_context(|| {
                                "invalid value for --min-shared-bits-between-outer-tables"
                            })?;
                }
                "--max-outer-pair-row-product" => {
                    args.max_outer_pair_row_product =
                        expect_value(&mut iter, "--max-outer-pair-row-product")?
                            .parse()
                            .with_context(|| "invalid value for --max-outer-pair-row-product")?;
                }
                "--help" | "-h" => {
                    print_usage();
                    std::process::exit(0);
    ...
    fn settings(&self) -> TriangleConsistencySettings {
        TriangleConsistencySettings {
            max_union_bits: self.max_union_bits,
            max_neighbors_considered: self.max_neighbors_considered,
            max_triangle_pairs_per_anchor: self.max_triangle_pairs_per_anchor,
            min_shared_bits_between_outer_tables: self.min_shared_bits_between_outer_tables,
            max_outer_pair_row_product: self.max_outer_pair_row_product,
        }
    }
}
...
struct RoundReport {
    ...
    filter: TriangleConsistencyInfo,
...
fn main() -> Result<()> {
    ...
    let report = json!({
        "method": "repeat bounded triangle consistency filtering until no further change",
        ...
        "total_anchors_with_triangle_pairs": rounds.iter().map(|round| round.filter.anchors_with_triangle_pairs).sum::<usize>(),
        "total_checked_triangle_pairs": rounds.iter().map(|round| round.filter.checked_triangle_pairs).sum::<usize>(),
        "total_informative_triangle_pairs": rounds.iter().map(|round| round.filter.informative_triangle_pairs).sum::<usize>(),
...
fn run_fixed_point_filter(
    ...
    loop {
        ...
        let (output_tables, info) = filter_tables_by_triangle_consistency(&tables, settings)?;
...
fn print_usage() {
    println!(
        "usage: cargo run --release --bin triangle_consistency_filter -- --input <path> [--output <path>] [--report <path>] [--max-rounds <n>] [--max-union-bits <n>] [--max-neighbors-considered <n>] [--max-triangle-pairs-per-anchor <n>] [--min-shared-bits-between-outer-tables <n>] [--max-outer-pair-row-product <n>]"
    );
}

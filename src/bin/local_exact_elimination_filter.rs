...
use tables::local_exact_elimination_filter::{
    filter_tables_by_local_exact_elimination_with_settings, LocalExactEliminationInfo,
    LocalExactEliminationSettings,
};
...
struct Args {
    ...
    max_tables_per_component: usize,
    min_tables_per_component: usize,
}

impl Default for Args {
    fn default() -> Self {
        let settings = LocalExactEliminationSettings::default();
        Self {
            input: PathBuf::from(
                "runs/2026-04-05-bounded-neighborhood-join-pipeline-aggressive-32x10/tables.fixed_point.json",
            ),
            output: PathBuf::from(
                "runs/2026-04-05-local-exact-elimination-filter/tables.fixed_point.json",
            ),
            report: PathBuf::from(
                "runs/2026-04-05-local-exact-elimination-filter/report.json",
            ),
            max_rounds: None,
...
impl Args {
    fn parse() -> Result<Self> {
        ...
        while let Some(flag) = iter.next() {
            match flag.as_str() {
                ...
                "--max-tables-per-component" => {
                    args.max_tables_per_component =
                        expect_value(&mut iter, "--max-tables-per-component")?
                            .parse()
                            .with_context(|| "invalid value for --max-tables-per-component")?;
                }
                "--min-tables-per-component" => {
                    args.min_tables_per_component =
                        expect_value(&mut iter, "--min-tables-per-component")?
                            .parse()
                            .with_context(|| "invalid value for --min-tables-per-component")?;
                }
                "--help" | "-h" => {
                    print_usage();
                    std::process::exit(0);
    ...
    fn settings(&self) -> LocalExactEliminationSettings {
        LocalExactEliminationSettings {
            max_union_bits: self.max_union_bits,
            max_tables_per_component: self.max_tables_per_component,
            min_tables_per_component: self.min_tables_per_component,
        }
    }
}
...
struct RoundReport {
    ...
    filter: LocalExactEliminationInfo,
...
fn main() -> Result<()> {
    ...
    let report = json!({
        "method": "repeat local exact elimination filtering until no further change",
        ...
        "total_exact_component_anchor_tables": rounds.iter().map(|round| round.filter.exact_component_anchor_tables).sum::<usize>(),
        ...
        "max_selected_union_bits": rounds.iter().map(|round| round.filter.max_selected_union_bits).max().unwrap_or(0),
        "max_selected_table_count": rounds.iter().map(|round| round.filter.max_selected_table_count).max().unwrap_or(0),
        "max_intermediate_row_count": rounds.iter().map(|round| round.filter.max_intermediate_row_count).max().unwrap_or(0),
        "rounds": rounds,
    });
...
fn run_fixed_point_filter(
    ...
    loop {
        ...
        let (output_tables, info) =
            filter_tables_by_local_exact_elimination_with_settings(&tables, settings)?;
...
fn print_usage() {
    println!(
        "usage: cargo run --release --bin local_exact_elimination_filter -- --input <path> [--output <path>] [--report <path>] [--max-rounds <n>] [--max-union-bits <n>] [--max-tables-per-component <n>] [--min-tables-per-component <n>]"
    );
}
